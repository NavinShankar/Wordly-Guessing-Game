...
function showScores(modal, type, diff){
	...
	for(i = 0; i < 4; i++){
		...
		modalScoreBlock.className = 'msBlock';
			let msHeadContent = (i == 0)? 'BEGINNER' : ((i == 1)? 'INTERMEDIATE' : ((i == 2)? 'ADVANCED' : 'LEGEND'));
...
function openModal(type, notification){
	...
	if(type == 'levelSelect'){
		for(i = 0; i < 5; i++){
			...
			modalBtn.innerText = (i == 0)? 'Beginner' : ((i == 1)? 'Intermediate' : ((i == 2)? 'Advanced' : ((i == 3)? 'LEGEND' : 'Custom')));
...
function startMenu(){
	...
	for(i = 0; i < 6; i++){
		...
		menuBtn.innerText = (i == 0)? maxBlock + ' LETTERS GAME' : ((i == 1)? level + ' LEVEL' : ((i == 2)? difficulty + ' MODE' : ((i == 3)? 'HIGH SCORES' : ((i == 4)? 'HOW TO PLAY' : 'START GAME'))));
		menuBtn.j = i;
